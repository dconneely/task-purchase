...
public class LiveRatesOfExchangeClient implements RatesOfExchangeClient {
    ...
    public LiveRatesOfExchangeClient(ClientProperties properties, RestTemplateBuilder restTemplateBuilder) {
     ...
     * @return RatesOfExchangeResponse the API response of exchange rate nearest to the transactionDate.
     */
    private RatesOfExchangeResponse fetchSingleRate(String countryCurrencyDesc, LocalDate transactionDate) {
        ...
        URI uri = UriComponentsBuilder.fromUriString(baseURL).path(ratesOfExchangeEndpoint)
                ...
                .queryParam("sort", "-effective_date").build().toUri();
        if (log.isDebugEnabled()) {
            log.debug("#fetchSingleRate(" + countryCurrencyDesc + ", " + transactionDate + "): request URI " + uri);
        }
        ResponseEntity<RatesOfExchangeResponse> response = restTemplate.getForEntity(uri, RatesOfExchangeResponse.class);
        if (!response.getStatusCode().is2xxSuccessful()) {
            log.info("#fetchSingleRate(" + countryCurrencyDesc + ", " + transactionDate + "): response error status " + response.getStatusCode());
