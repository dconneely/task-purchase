...
public class PurchaseServiceTest {
  ...
  public void testCreate() {
    ...
    when(repository.save(any(PurchaseEntity.class)))
        .thenAnswer(
            input -> {
  ...
  public void testGet() {
    ...
    PurchaseConverter converter = mock(PurchaseConverter.class);
    when(converter.responseFromEntity(any(PurchaseEntity.class), any(BigDecimal.class)))
        .thenCallRealMethod();
    when(converter.convertedAmount(any(BigDecimal.class), any(BigDecimal.class)))
        .thenCallRealMethod();
    RatesOfExchangeClient client = mock(RatesOfExchangeClient.class);
    when(client.getSingleRate(anyString(), any(LocalDate.class))).thenReturn(EXCHANGE_RATE_GOOD);
